...
def main():
    ...
    # 当前网络需要的 triple shape：
    # 第一层 / ReLU / 第一层 trunc：shape = (batch, 2)
    # 第二层 / 第二层 trunc：shape = (batch, 1)
    arith_plan = [
        ((batch, 2), 100),
        ((batch, 1), 100),
    ]
    ...
    with time_block("total_time"):
        ...
        # linear 后会先到 2f scale，再 trunc，所以 bias 编码为 2f
    ...
    print("===== Batch Fixed-point Secret MLP Test =====")
    print("X_plain =")
    print(X_plain)
    print("H_expected =")
    print(H_expected)
    ...
    if np.all(diff <= 1.0 / (1 << scale_bits)):
        print("Batch MPC test PASSED ✅")
    else:
        print("Batch MPC test FAILED ❌")

    print_report("Party0 Batch Offline / Online Profiler")
